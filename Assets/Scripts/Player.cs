...
public class Player : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        // wechselt die Richtung abhängig danach in welchem Bereich der Kopf detektiert wird
        if (faceDetector.faceX <= 0.28)
        {
            // Kopf ist im linken Bereich --> Bewegung nach links
            towardsY = -45f;
            transform.position += speed * Vector3.forward;
            transform.position += speed * transform.forward;
        }
        else if (faceDetector.faceX >= 0.55)
        {
            // Kopf ist im rechten Bereich --> Bewegung nach rechts
            towardsY = -135f;
        ...
        {
            // ansonsten ist Kopf im mittleren Bereich --> Bewegung geradeaus
            towardsY = -90f;
            transform.position += speed * transform.forward;
        }
